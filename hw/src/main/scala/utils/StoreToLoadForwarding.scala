...
import chisel3._
import chisel3.util.{MuxCase, RegEnable, ShiftRegister, ValidIO}
...
class StoreToLoadForwardingWithStallIO[T <: Data](gen: T, addrWidth: Int)
  extends StoreToLoadForwardingIO(gen, addrWidth) {
  val stall = Output(Bool())
  val rdAddrValid = Input(Bool())
}
